...
public abstract class ExtendableListView extends AbsListView {
    ...
    private GridRefreshListener mGridRefreshListener;

    public final void setOnGridRefreshListener(GridRefreshListener listener) {
        mGridRefreshListener = listener;
    }
        ...
        try {
            ...
            if (mItemCount == 0) {
               ...
               /* throw new IllegalStateException("The content of the adapter has changed but "
                ...
                Log.e("IllegalStateException", "The content of the adapter has changed but "
                        + "ExtendableListView did not receive a notification. Make sure the content of "
                        + "your adapter is not modified from a background thread, but only "
                        + "from the UI thread. [in ExtendableListView(" + getId() + ", " + getClass()
                        + ") with Adapter(" + mAdapter.getClass() + ")]");

                if (mGridRefreshListener != null) {
                    mGridRefreshListener.onGridRefresh();
                }
...
//            case TOUCH_MODE_OVERSCROLL:
    ...
    class AdapterDataSetObserver extends DataSetObserver {
        ...
        public void onChanged() {
            mDataChanged = true;
            mOldItemCount = mItemCount;
            mItemCount = ExtendableListView.this.mAdapter.getCount();
